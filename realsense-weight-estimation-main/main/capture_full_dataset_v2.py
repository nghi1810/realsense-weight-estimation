...
folders = ["RGB_full", "RGB_in_BB", "depth_full", "depth_in_BB",
...
while True:
    ...
    if key == ord('e'):
        ...
        # ===== LƯU depth full =====
        cv2.imwrite(os.path.join(save_dir, "depth_full", f"Depth_full_{timestamp}.png"), depth_image)

        # ===== LƯU depth BB =====
        depth_bb = depth_image[y1:y2, x1:x2]
        cv2.imwrite(os.path.join(save_dir, "depth_in_BB", f"Depth_in_BB_{timestamp}.png"), depth_bb)

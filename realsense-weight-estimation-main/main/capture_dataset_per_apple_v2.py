...
# ===== ĐÃ SỬA: TẠO CÁC THƯ MỤC CON ĐỂ KHÔNG BỊ LỖI KHI LƯU FILE =====
sub_dirs = [
    "RGB_full", "RGB_in_BB",
...
while True:
    ...
    if key == ord('e'):
        ...
        # Chuẩn hóa ảnh depth về 8-bit để hiển thị (không bị đen thui)
        depth_image_visual = cv2.normalize(depth_image, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)

        # ===== LƯU RGB full =====
        cv2.imwrite(os.path.join(save_dir, "RGB_full", f"RGB_full_{timestamp}.png"), img)

        # ===== LƯU RGB BB =====
        try:
            rgb_bb = img[y1:y2, x1:x2]
            cv2.imwrite(os.path.join(save_dir, "RGB_in_BB", f"RGB_in_BB_{timestamp}.png"), rgb_bb)
        except NameError:
            pass # Bỏ qua nếu không có bounding box

        # ===== LƯU depth full =====
        cv2.imwrite(os.path.join(save_dir, "depth_full", f"Depth_full_{timestamp}.png"), depth_image_visual)

        # ===== LƯU depth BB =====
        try:
            depth_bb_visual = depth_image_visual[y1:y2, x1:x2]
            cv2.imwrite(os.path.join(save_dir, "depth_in_BB", f"Depth_in_BB_{timestamp}.png"), depth_bb_visual)
        except NameError:
            pass
        ...
        ply_full_file = os.path.join(save_dir, "PLY_full", f"PLY_full_{timestamp}.ply")
        ...
        try:
            ...
            ply_bb_file = os.path.join(save_dir, "PLY_in_BB", f"PLY_in_BB_{timestamp}.ply")
        ...
        # ===== LƯU PLY contour với Z filter 0.6-0.7 =====
        try:
            if 'contours' in locals() and len(contours) > 0:
                ...
                ply_contour_file = os.path.join(save_dir, "PLY_in_contour", f"PLY_in_contour_{timestamp}.ply")
        ...
        # ===== LƯU npy full =====
        np.save(os.path.join(save_dir, "npy_full", f"full_{timestamp}.npy"), full_points)

        # ===== LƯU npy BB =====
        try:
            np.save(os.path.join(save_dir, "npy_in_BB", f"in_BB_{timestamp}.npy"), bb_points)
        except NameError:
            pass

        # ===== LƯU npy contour với Z filter =====
        # ===== LƯU npy contour với Z filter 0.6-0.7 =====
        try:
            if 'contours' in locals() and len(contours) > 0:
                contour_points_npy = np.array([p for p in contour_points if 0.6 < p[2] <= 0.68])
                np.save(
                    os.path.join(save_dir, "npy_in_contour", f"in_contour_{timestamp}.npy"),
                    contour_points_npy
                )
        except NameError:
            pass

        print(f"[+] Saved all data for {timestamp}")
